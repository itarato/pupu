...
struct Map {
 public:
      ...
      switch (tile_selection.source) {
        ...
        case TileSource::Trap5:
          interactive_objects.push_back(
              std::make_shared<DisappearingPlank>(pixel_size, tile_pos.scale(pixel_size).to_vector2()));
          break;
        default:
          BAIL;
      }
  ...
  bool check_collision_with_fully_solid_walls(Rectangle const& collider) const {
    if (leftx(collider) < 0.f || rightx(collider) > get_world_width() || topy(collider) < 0.f ||
        bottomy(collider) > get_world_height())
      return true;
  ...
  void check_south_collision(CollisionResult* out, Rectangle const& map_object_hitbox, Rectangle const& collidee_hitbox,
    ...
    if (is_horizontal_overlap(map_object_hitbox, collidee_hitbox)) {
      if (topy(map_object_hitbox) < out->wall &&
        ...
        out->wall_vertical_speed = ydelta;
      }
    }
  }
